...
void PrintHelpInfo(std::shared_ptr<DumpContext> dumpContext)
{
...
void PrintErrorMessage(std::shared_ptr<DumpContext> dumpContext, const std::string& errorMsg)
{
...
DumpStatus CmdParse::Parse(int argc, char *argv[], std::shared_ptr<DumpContext> dumpContext)
{
    if (CheckArgs(argc, argv) != DumpStatus::DUMP_OK || dumpContext == nullptr) {
...
DumpStatus CmdParse::CmdParseHelper(int argc, char *argv[], std::shared_ptr<DumpContext> dumpContext)
{
...
DumpStatus CmdParse::ParseLongCmdOption(int argc, std::shared_ptr<DumpContext> dumpContext,
...
bool CmdParse::ParseSubLongCmdOption(int argc, std::shared_ptr<DumpContext> dumpContext,
                                     const struct option longOptions[], const int &optionIndex, char *argv[])
{
...
void CmdParse::CheckIncorrectCmdOption(const char *optStr, char *argv[], std::shared_ptr<DumpContext> dumpContext)
{
    if (optopt == 0) {
...
DumpStatus CmdParse::ParseShortCmdOption(int c, std::shared_ptr<DumpContext> dumpContext, int argc, char *argv[])
{
    switch (c) {
        case 'a':
...
bool CmdParse::IsShortOptionReqArg(const char *optStr, std::shared_ptr<DumpContext> dumpContext)
{
...
DumpStatus CmdParse::SetCmdParameter(int argc, char *argv[], std::shared_ptr<DumpContext> dumpContext)
{
    ...
    if (optind > 1 && optind <= argc) {
        ...
        if (IsHiviewEnabled() && prevArg == "--cpuusage") {
            ...
            dumpContext->GetDumperOpts()->abilitieNames.insert(currentArg);
        } else if (prevArg == "--ipc") {
...
DumpStatus CmdParse::SetCmdIntegerParameter(const std::string &str,
                                            int &value, std::shared_ptr<DumpContext> dumpContext)
{
    if (!IsNumericStr(str)) {
...
DumpStatus CmdParse::SetMemJsheapParam(std::shared_ptr<DumpContext> dumpContext)
{
    if (optarg == nullptr) {
...
DumpStatus CmdParse::SetRawParam(std::shared_ptr<DumpContext> dumpContext)
{
...
bool CmdParse::SetIpcStatParam(std::shared_ptr<DumpContext> dumpContext, const std::string& param)
{
    if (param == "start-stat") {
...
DumpStatus CmdParse::ParseCmdOptionForA(std::shared_ptr<DumpContext> dumpContext, char *argv[])
{
    if (dumpContext->GetDumperOpts()->isDumpSystemAbility) {
        SplitStr(optarg, " ", dumpContext->GetDumperOpts()->abilitieArgs);
...
DumpStatus CmdParse::CheckProcessAlive(std::shared_ptr<DumpContext> dumpContext)
{
...
DumpStatus CmdParse::VerifyOptions(const std::shared_ptr<DumpContext>& dumpContext)
{
    auto opts = dumpContext->GetDumperOpts();
    ...
    for (const auto& name : opts->abilitieNames) {
