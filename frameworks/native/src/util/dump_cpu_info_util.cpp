...
bool DumpCpuInfoUtil::GetCurCPUInfo(std::shared_ptr<CPUInfo> &cpuInfo)
{
    ...
    bool ret = fileInstance.LoadStringFromProcCb(PROC_STAT_PATH, false, false, [&](const std::string& line) -> void {
        statRawData += line;
    });
    if (!ret) {
        return false;
...
bool DumpCpuInfoUtil::GetCurProcInfo(std::vector<std::shared_ptr<ProcInfo>> &procInfos)
{
    ...
    for (size_t i = 0; i < procFiles.size(); i++) {
        std::string rawData;
        bool ret = fileInstance.LoadStringFromProcCb(procFiles[i], false, false, [&](const std::string& line) -> void {
            rawData += line;
        });
        if (!ret) {
            continue;
...
bool DumpCpuInfoUtil::GetCurSpecProcInfo(int pid, std::shared_ptr<ProcInfo> &specProc)
{
    ...
    std::string rawData;
    bool ret = FileUtils::GetInstance().LoadStringFromProcCb(path, false, false, [&](const std::string& line) -> void {
        rawData += line;
    });
    if (!ret) {
        return false;
