...
DumpStatus CollectRootTasks(const DumpContext& context, std::vector<TaskId>& rootTasks) override
{
    ...
    if (opt->isDumpList) {
        rootTasks.push_back(TaskId::WRITE_SYSTEM_CLUSTER_INFO);
        return DUMP_OK;
    }
    ...
    addTaskIfNeeded("base", { TaskId::WRITE_SYSTEM_BASE_INFO });
    addTaskIfNeeded("system", { TaskId::WRITE_KERNEL_MODULE_INFO, TaskId::WRITE_KERNEL_MEM_INFO,
                                TaskId::WRITE_CPU_FREQ_INFO, TaskId::WRITE_ALL_PROCESS_MEM_INFO });
    return DUMP_OK;
}
};
} // namespace HiviewDFX
