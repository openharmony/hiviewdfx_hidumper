...
public:
    ...
    DumpContext dumpContext_ = DumpContext(getpid(), getuid(), STDOUT_FILENO);
};
...
HWTEST_F(StorageInfoTaskTest, DiskInfoTaskSuccess, TestSize.Level1)
{
    ...
    EXPECT_TRUE(data->at(0).find("Filesystem") != std::string::npos);
}
...
HWTEST_F(StorageInfoTaskTest, IoTopInfoTaskSuccess, TestSize.Level1)
{
    ...
    ASSERT_NE(data, nullptr);
    ASSERT_FALSE(data->empty());
    EXPECT_TRUE(data->at(0).find("Totals:") != std::string::npos);
}
...
HWTEST_F(StorageInfoTaskTest, LsofInfoTaskSuccess, TestSize.Level1)
{
    ...
    ASSERT_NE(data, nullptr);
    ASSERT_FALSE(data->empty());
    EXPECT_TRUE(data->at(0).find("COMMAND") != std::string::npos);
}
...
{
    string path = "/proc/" + to_string(INIT_PID) + "/io";
